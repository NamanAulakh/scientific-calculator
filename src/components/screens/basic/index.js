...
import * as setActions from "../../../redux/settings/actions";
...
class Basic extends Component {
  ...
  handleSlot2(item) {
    ...
    if (item === "AC")
      return this.setState({ inputValue: "", outputValue: "" });
    ...
    if (item === "(" || item === ")") {
      return this.setState({ inputValue: inputValue.concat(item) }, () =>
        this.calculate()
      );
    }
  ...
  calculate() {
    ...
    if (!includes(inputValue, "sec"))
      inputValue = inputValue.replace(/e/g, `${math.e}`);
    // inputValue = inputValue.replace(/asin/g, `math.asin`)


    try {
      let res = math.evaluate(inputValue);
      if (typeof res === "object") res = JSON.stringify(math.evaluate(inputValue));
      ...
      this.setState(
        ...
        () => {
          ...
          if (parseInt(JSON.stringify(res))) {
            return setHistory(
              `${this.state.inputValue} = ${JSON.stringify(res)}`
            );
          }
        }
      );
  ...
  handleOtherOps(item) {
    ...
    if (item === "ln") {
      return this.setState({ inputValue: inputValue.concat("log(") }, () =>
        this.calculate()
      );
    }
    if (item === "log10b") {
      return this.setState({ inputValue: inputValue.concat("log10(") }, () =>
        this.calculate()
      );
    }
    if (
      ...
      if (isUnitDegrees) {
        return this.setState(
          { inputValue: inputValue.concat(`${item}((π/180)*`) },
          () => this.calculate()
        );
      }
    ...
    if (item === "nCr" || item === "nPr")
      return this.setState({ modalVisible: true, func: item });
    ...
    return (
      <View style={container}>
        ...
        <View style={content}>
          ...
          <Operations
            handleMainOp={this.handleMainOp}
            handleOtherOps={this.handleOtherOps}
          />
